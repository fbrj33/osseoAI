...
# PATCH 1: Returns doctor_name, xray_count, analysis_count — was missing all three
...
# PATCH 2: New endpoint — frontend modal calls GET /api/patients/:id/analyses
@app.get("/api/patients/{patient_id}/analyses")
def get_patient_analyses(patient_id: int, doctor: Doctor = Depends(get_current_doctor)):
    """Return all analyses for a patient, used by the patient file modal."""
    _require_approved(doctor)
    db = SessionLocal()

    # Admins can access any patient; doctors only their own
    query = db.query(Patient).filter(Patient.id == patient_id)
    if doctor.role != "admin":
        query = query.filter(Patient.doctor_id == doctor.id)
    p = query.first()
    ...
    for a in analyses:
        ...
        study_image_count = (
            db.query(Xray)
            .filter(Xray.patient_id == patient_id, Xray.study_uid == a.study_uid)
            .count()
...
def get_history(
    ...
    # Admins see all analyses across all doctors; regular doctors see only their own
    if doctor.role == "admin":
        rows = (
            db.query(Analysis)
            .order_by(Analysis.created_at.desc())
...
# ═══════════════════════════════════════════════════════════
# STATS  — PATCH 3: was missing entirely, caused total_patients = 0
# ═══════════════════════════════════════════════════════════

@app.get("/api/stats")
def get_stats(doctor: Doctor = Depends(get_current_doctor)):
    """Summary stats for the Reports page."""
    _require_approved(doctor)
    db = SessionLocal()

    # Admins see global stats; regular doctors see only their own
    if doctor.role == "admin":
        patient_count  = db.query(Patient).count()
        analysis_count = db.query(Analysis).count()
        fracture_count = db.query(Analysis).filter(Analysis.fracture == True).count()
    else:
        pid_sub        = db.query(Patient.id).filter(Patient.doctor_id == doctor.id).subquery()
        patient_count  = db.query(Patient).filter(Patient.doctor_id == doctor.id).count()
        analysis_count = db.query(Analysis).filter(Analysis.patient_id.in_(pid_sub)).count()
        fracture_count = db.query(Analysis).filter(
            Analysis.patient_id.in_(pid_sub),
            Analysis.fracture == True,
        ).count()
    ...
    return {
        ...
        "total_fractures": fracture_count,
        "normal_count":    analysis_count - fracture_count,
    }
